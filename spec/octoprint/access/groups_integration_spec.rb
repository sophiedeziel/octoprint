...
RSpec.describe Octoprint::Access::Groups, vcr: { cassette_name: "access/groups" } do
  describe ".list" do
    it "fetches groups from the API" do
      ...
      expect(group_keys).to include("admins", "operators", "users")
  ...
  describe "CRUD operations" do
    let(:test_group_key) { "test_group_#{Time.now.to_i}" }
    ...
    it "creates, updates, and deletes a group" do
      ...
      expect(created_group.permissions).to eq(["MONITOR"])
      ...
      updated_group = described_class.update(
        ...
        params: {
          ...
          permissions: %w[MONITOR FILES_LIST]
        }
      )
